...
**This epic builds infrastructure only — zero screens.** Epics 9-11 build the actual pick-flow, standings, and league-management UI. Every module below exists so those epics can build screens without first solving clock correctness, cache/polling policy, optimistic-write safety, offline behavior, or deep-linking from scratch — the same "solve it once, correctly, before anyone needs it under deadline" posture this repo has taken with every prior epic.
...
- What a `PickRejection`/queued-and-unsaved pick actually *looks like* on screen — this epic guarantees the DATA is there (`rejection`, `isQueued(gameId)`) and that it can't be silently dropped; the visual treatment is a design decision.
- Any UI framework/component library, styling approach, or design system.
- The home screen, league-management screens, standings views, etc. — none of `src/query/hooks/` beyond `useSlate` were built out further than what this epic's own modules needed to demonstrate/test.
