...
| `src/screens/marketing/` | The logged-out marketing chrome — `MarketingHeader` (sticky nav), `MarketingSections` (proof band, how it works, the sport list, features, the no-account quiz callout, FAQ, closing CTA), `MarketingFooter`, over one shared `Marketing.module.css` and a JSX-free `marketing-content.ts` holding every word of copy. Mounted today by `screens/auth/LoginScreen` — `/login` is a full landing page with the login card in its hero, not a form on a background. Extracted rather than inlined so `PublicHomeScreen` (`/`, the other logged-out front door) can adopt the identical sections instead of forking them; it still runs its own shorter hero. Every claim in `marketing-content.ts` cites the doc or module it comes from, and the sport grid renders `leagues/sports.ts`'s real `SPORT_OPTIONS` so the page can't advertise a sport the server would reject. |
