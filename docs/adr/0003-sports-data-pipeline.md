...
- No published rate limit. Mitigated by conservative call volume by design: `schedule-ingest` runs every 4 hours (not more often) and issues one call per sport per run (9 calls/run, plus a small number of targeted re-fetches for postponed games); `score-poll` runs every 5 minutes but only fetches games whose start has already passed and whose status isn't final, so a quiet stretch with nothing live costs zero calls.
