...
- **Golf added** in a still-later session, once the user specified the mechanic (pick N golfers before the tournament; any top-N finish is a win; unrestricted golfer overlap across members; grading updates live as the leaderboard moves). Golf deliberately does **not** join `ESPN_SPORT_SLUGS` or this adapter at all — the "genuinely different pick mechanic" flagged above was built as its own parallel stack instead: `lib/golf-provider.ts` (its own adapter over `golf/pga`), `tournament`/`tournament_entry`/`golf_pick`/`golf_pick_selection` tables, one combined `golf-ingest` job, and `lib/golf-grading.ts`'s deliberately re-runnable grading. Confirmed live against a mid-Round-2 tournament: one event, one competition, ~69 `athlete` competitors, `order` giving direct leaderboard rank (1 = leader), and **no CUT/WD marker anywhere in the payload** — `order` is the only position signal ESPN exposes here. `event.date`/`event.endDate` give the tournament's real span, which is what the pick lock and the standings credit day are computed from. See `docs/sports-pipeline.md`'s Golf section.
