 ...
 * leagues.routes.ts purely for size; same `/leagues` prefix.
 * See docs/leagues-and-membership.md.
 *
 * NOT one blanket `authenticate` hook for the whole file as originally
 * shipped — a real, confirmed gap found building Epic 11's join
 * screens: `GET /preview` required auth exactly like every other route
 * here, but the brief is explicit that the deep-link flow is
 * "preview -> signup -> auto-join" for a visitor who has NO account
 * yet at all. `POST /join` genuinely can't be made public (it attaches
 * a membership row to a real userId), so the fix is scoped to
 * `/preview` alone: two separate nested plugin scopes below, one
 * still fully `authenticate`-gated (invite-code view/rotate, join),
 * one using `optionalAuthenticate` (preview) so an already-logged-in
 * caller still gets an accurate `alreadyMember` while a logged-out one
 * isn't turned away.
 */
export async function leagueInvitesRoutes(app: FastifyInstance): Promise<void> {
  await app.register(async (instance) => {
    instance.addHook("preHandler", authenticate);
    await registerAccountRateLimit(instance);
    ...
    /**
     * `/join` is a "guess an invite code" endpoint — a tight, PER-USER
     * limit here specifically slows code-guessing, on top of (not
     * instead of) the app-wide per-IP limit (app.ts) and the general
     * per-account limit registered above. Nested plugin registration,
     * not a route-level `config.rateLimit` override: that field is read
     * by EVERY listening `@fastify/rate-limit` registration's own
     * `onRoute` hook, so two independent registrations can't each
     * express a different config through the same shared field —
     * confirmed empirically while fixing what used to be here
     * (`config.rateLimit: {max:20}` plus an explicit `preHandler:
     * app.rateLimit({max:10, keyGenerator: perUser})`), which looked
     * like two layered limits but was actually one working per-IP check
     * and one COMPLETELY INERT per-user check — every rate-limit check
     * derived from the same `@fastify/rate-limit` registration shares
     * one single-fire-per-request guard (see lib/rate-limit.ts's
     * comment for the full mechanism), so the second check never even
     * evaluated, let alone rejected anything. A genuinely independent
     * check needs its own registration, which is what this nested
     * plugin provides.
     */
    await instance.register(async (joinScope) => {
      await joinScope.register(rateLimit, {
        max: 10,
        timeWindow: "1 minute",
        hook: "preHandler",
  ...
  /**
   * Public (via `optionalAuthenticate`, not `authenticate`) — the
   * fix described in this file's own top comment. Rate-limited per-IP
   * rather than the `registerAccountRateLimit`/per-user scheme above,
   * since there's no account at all for the case this exists to serve
   * (a fresh visitor previewing a real invite link before signing up);
   * the app-wide per-IP limit (app.ts) still applies underneath this
   * one, same "on top of, not instead of" layering the /join limit
   * above already established for this file.
   */
  await app.register(async (instance) => {
    instance.addHook("preHandler", optionalAuthenticate);
    ...
    instance.get(
      ...
      async (request) => {
        ...
        // No account at all (a fresh visitor) can't already be a
        // member — skip the query entirely rather than querying with
        // an undefined userId.
